...
def inbox(request):
    ...
    merchants = User.objects.filter(profile__is_merchant=True).exclude(
        pk=request.user.pk
    )
...
def conversation_detail(request, conversation_id):
    ...
    context = {
        "conversation": conversation,
        "other_user": conversation.other_participant(request.user),
        "message_list": conversation.messages.select_related("sender").prefetch_related(
            "attachments"
        ),
    }
...
def post_announcement(request, store_id):
    ...
    announcement = Announcement.objects.create(
        store=store, author=request.user, content=content
    )
