...
def add_members(request, store_id):
    ...
    if not user_membership or user_membership.role != "Owner":
        messages.error(
            request, "You do not have permission to add members to this store."
        )
        return redirect("create_store")
        ...
        if "send_invitation_btn" in request.POST:
            ...
            if membership:
                ...
                if membership_form.is_valid():
                    ...
                    messages.error(
                        request, "Error updating membership. Please try again."
                    )
            else:
...
def my_job_invitations(request):
    recieved_invitations = MembershipInvitation.objects.filter(
        invitee_email=request.user.email, status="Pending"
    )
...
def view_orders(request, store_id):
    ...
    if request.method == "POST":
        ...
        order = (
            Order.objects.filter(id=order_id, orderitem__product__store=store)
            .distinct()
            .first()
        )
        if not order:
...
def sign_as_delivery(request):
    ...
    if request.method == "POST":
        ...
        profile.save()
        messages.success(
            request, "You have successfully signed up as a delivery person."
        )
...
def view_available_parcels(request):
    ...
    available_orders = Order.objects.filter(
        delivery_person__isnull=True, status="Processing"
    )
